...
<div x-data="loggedCloudMultiGrid({{ \Illuminate\Support\Js::from($config) }})"
    ...
    <template x-for="(opt, i) in items" :key="opt.key">
        <button type="button"
            ...
            <span class="lc-multi-grid__icon" aria-hidden="true" x-show="opt.svg">
            ...
            <span class="lc-multi-grid__title" x-text="opt.title"></span>
            <span class="lc-multi-grid__subtitle" x-show="opt.subtitle" x-text="opt.subtitle"></span>
        </button>
    </template>
    ...
    @endonce
    @once
        <script data-lc-multi-grid-alpine>
            (function () {
                if (window.__loggedCloudMultiGridLoaded) return;
                window.__loggedCloudMultiGridLoaded = true;

                document.addEventListener('alpine:init', () => {
                    window.Alpine.data('loggedCloudMultiGrid', (config) => ({
                        items: config.items || [],
